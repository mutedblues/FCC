...
let filteredList = watchList.filter(elem => Number(elem.imdbRating) >= 8.0).map(elem => ({title: elem.Title, rating: elem.imdbRating}));
...
Basically, the above does the same as this longer version:
...
watchList.filter(function(elem){
  if(Number(elem.imdbRating) >= 8.0){
    return elem;
  }
}).map(function(elem){
  filteredList.push({title: elem.Title, rating: elem.imdbRating});
});
